...
class WeatherApp:
    def _init_(self, root, api_key, app_config):
        ...
        # Set custom icon
        self.root.iconbitmap ( 'orange_eyes.ico' )
        ...
        # Initialize with default city (Lagos)
        self.city_entry.insert ( 0, self.city_entry.get() )
        self.fetch_and_display_current_weather ()

        # Fetch and plot historical weather data for Lagos upon startup
        # self.fetch_and_plot_historical_data ()
    ...
    def fetch_and_display_current_weather(self):
        ...
        self.save_weather_data_to_db ( city, weather_data )
        ...
        self.current_weather_label.delete ( 1.0, tk.END )  # Clear previous text
        self.current_weather_label.insert ( tk.END, weather_info )

        # Check for alerts
        self.check_alerts ( city, weather_data['main']['temp'] )
        historical_data = self.fetch_historical_data ( self.db_config, city=self.city_entry.get () )
        self.plot_historical_data ( historical_data )

    # def fetch_and_plot_historical_data(self):
    #     historical_data = self.fetch_historical_data ( self.db_config, city=self.city_entry.get())
    #     self.plot_historical_data ( historical_data )
    ...
    def save_weather_data_to_db(self, city, weather_data):
    ...
    def set_alert(self):
        ...
        # Insert alert data into the Alerts table
    ...
    def check_alerts(self, city, current_temp):
        ...
        for threshold, message in alerts:
            if current_temp >= threshold:
                ...
                break  # Only show the first alert that matches
...
db_config = {
    'host': 'localhost',
    'user': 'root',
    'password': 'Nou233324166/',
    'database': 'weatherdb'
}

api_key = 'dd76448b732fbbf48a70b0511750dde5'

# Initialize and run the Tkinter app
root = tk.Tk ()
app = WeatherApp ( root, api_key, db_config )
root.mainloop ()